...
Enemy::~Enemy()
{
	if (collider != nullptr)
	{
		...
		if (life <= 0 && enemyType != HOMINGMISSILE && enemyType != BIGFUCKINGROCKET && enemyType != SHURIKEN) //only spawn powerup if the enemy is really death
		{
		...
		if (enemyType == SHURIKEN)
		{
			App->particles->AddParticle(App->enemies->shurikenDeath, position.x, position.y, COLLIDER_NONE);
		}
