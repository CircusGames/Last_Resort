...
ModulePlayer2Unit::ModulePlayer2Unit()
{
...
update_status ModulePlayer2Unit::PreUpdate()
{
	...
	if (!unitLocked)
	{
		...
		if (App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_REPEAT &&
			...
			App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_IDLE) //constrains upper left diagonal case
		{
		...
		if (App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_REPEAT && //if right
			App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_IDLE &&   //and not left (original opposite direction priority)
			App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_IDLE &&   //and not up (diagonal case)
			App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_IDLE)     //and not down right
		{
		...
		if (App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_REPEAT &&
			App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_IDLE   && //constrains original opposite up and down case
			App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_IDLE   &&
			App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_IDLE)    //constrains upper left diagonal case
		{
		...
		if (App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_REPEAT &&
			App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_IDLE && //constrains diagonal down and left
			App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_IDLE) //constrains diagonal down and right
		{
...
update_status ModulePlayer2Unit::Update()
{
	...
	//if we press down
	if (App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_REPEAT &&
		App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_IDLE) //original game behaviour priority
	{
	...
	if (App->input->keyboard[SDL_SCANCODE_RIGHT] == KEY_STATE::KEY_REPEAT &&
		App->input->keyboard[SDL_SCANCODE_LEFT] == KEY_STATE::KEY_IDLE) //double check for original game priority
	{
	...
	playerPos.x = (App->player2->position.x - 7) + cos(angle) * 31.5f;//orginal distance // 9
	playerPos.y = (App->player2->position.y - 7) - sin(angle) * 32;//24; // 7
...
bool ModulePlayer2Unit::CleanUp()
{
	//unload textures
